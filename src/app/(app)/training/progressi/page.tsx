...
import { Search, ChevronLeft, TrendingUp, Dumbbell, Plus, X, Loader2, Check } from 'lucide-react'
...
function AddExModal({ userId, templateId, onClose, onAdded }: {
  userId: string; templateId: string; onClose: () => void; onAdded: () => void
}) {
  ...
  async function handleSave() {
    ...
    await fetch('/api/template-exercises', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ templateId, exerciseId: selected.id, sets: 3, reps: '10', restSeconds: 90 }),
    })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm bg-black/40 px-4">
      <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-md max-h-[80vh] flex flex-col shadow-xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3.5 border-b border-gray-100 dark:border-gray-800 shrink-0">
          <div className="w-8" />
          <p className="font-bold text-gray-900 dark:text-gray-100 flex-1 text-center">Aggiungi esercizio</p>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
          {!selected ? (
            <>
                ...
                <div className="border border-gray-100 dark:border-gray-800 rounded-2xl overflow-hidden">
                  ...
                  {q.length >= 2 && !searching && (
                    <button onClick={addToDb} disabled={creating}
                      ...
                      <div className="w-6 h-6 rounded-lg flex items-center justify-center shrink-0" style={{ backgroundColor: C.training + '20', color: C.training }}>
                        {creating ? <Loader2 size={12} className="animate-spin" /> : <Plus size={12} />}
                      </div>
                      <p className="text-sm font-semibold" style={{ color: C.training }}>Aggiungi &ldquo;{q}&rdquo;</p>
                    </button>
                  )}
                </div>
              )}
              {q.length >= 2 && !searching && results.length === 0 && (
                <div className="border border-gray-100 dark:border-gray-800 rounded-2xl overflow-hidden">
                  ...
                  <button onClick={addToDb} disabled={creating}
                    className="w-full flex items-center gap-3 px-4 py-3 border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                    <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0" style={{ backgroundColor: C.training + '20', color: C.training }}>
                      {creating ? <Loader2 size={13} className="animate-spin" /> : <Plus size={13} />}
                    </div>
                    <p className="text-sm font-semibold" style={{ color: C.training }}>Aggiungi &ldquo;{q}&rdquo; al database</p>
                  </button>
                </div>
              )}
            ...
            <div className="flex items-center justify-between rounded-xl px-3 py-2.5" style={{ backgroundColor: C.training + '15' }}>
        ...
        {selected && (
          <div className="px-4 pb-5 pt-3 border-t border-gray-100 dark:border-gray-800 shrink-0">
            <button onClick={handleSave} disabled={saving}
              ...
              style={{ backgroundColor: C.training }}>
...
export default function ProgressiPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto md:max-w-none space-y-3">
      ...
      {step === 'exercise' && (
        <>
          ...
          {loading ? (
            ...
            <button onClick={() => setShowAddEx(true)}
              className="w-full flex flex-col items-center gap-2 py-10 rounded-2xl border-2 border-dashed text-sm font-medium transition-colors"
              style={{ borderColor: C.training + '60', color: C.training }}>
              <Plus size={20} />
      ...
      {showAddEx && selTemplate && (
        <AddExModal
          userId={userId}
          templateId={selTemplate.id}
          onClose={() => setShowAddEx(false)}
          onAdded={() => { setShowAddEx(false); loadExercises() }}
        />
      )}
