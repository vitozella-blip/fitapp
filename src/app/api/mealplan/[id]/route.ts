...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (targets) {
      for (const [meal, t] of Object.entries(targets as Record<string, { calories: number; protein: number; carbs: number; fat: number }>)) {
        await pool.query(
          `UPDATE "MealPlanTarget" SET calories=$1, protein=$2, carbs=$3, fat=$4 WHERE "planId"=$5 AND meal=$6`,
          [t.calories, t.protein, t.carbs, t.fat, id, meal]
        )
      }
    }
